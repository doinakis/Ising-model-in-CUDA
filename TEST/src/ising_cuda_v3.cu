...
void ising_kernel(int *G, int *new_G, double *w, int n){
  ...
  __shared__ int shared_G[(NUMTHREADS+4)*(NUMTHREADS+4)];
  ...
  for(int ip = ip0; ip < n; ip += blockDim.x*gridDim.x){
    for(int jp = jp0; jp < n; jp += blockDim.y*gridDim.y){
      ...
      for(int i = threadIdx.x; i < NUMTHREADS + 4; i+=NUMTHREADS){
        for(int j = threadIdx.y; j < NUMTHREADS + 4; j+=NUMTHREADS){
          shared_G(i,j) = G((ip - threadIdx.x + i - 2 + n) % n, (jp - threadIdx.y + j - 2 + n) % n);
        }
      }
      __syncthreads();
      ...
      for(int in = -2; in <= 2; in++){
        for(int jn = -2; jn <= 2; jn++){
          ...
          // add weighted spins
          weighted_sum += shared_w(in + 2 , jn + 2) * shared_G(threadIdx.x + in + 2, threadIdx.y + jn + 2);
